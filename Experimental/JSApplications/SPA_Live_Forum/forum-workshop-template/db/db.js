...
function threadsAddMessage(id, msg) {
  ...
  var promise = new Promise(function(resolve, reject) {
       var searchedThread;
       for (var i = 0, len = storage.threads.length; i < len; i += 1) {
          if (storage.threads[i].id === id) {
           	  searchedThread = storage.threads[i];
              return;
          }
      }

      console.log('DB.JS found thread ' + searchedThread);
      if (!searchedThread) {
        throw new Error('invalid thread id');
      }

      msg.postDate = new Date();
      searchedThread.messages.push(msg);

      console.log('DB.JS after update ' + searchedThread.messages);

      resolve(searchedThread);
  });
